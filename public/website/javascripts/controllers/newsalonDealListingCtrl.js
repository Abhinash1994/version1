app.controller('newsalomDealCtrl', ['$rootScope','$scope', '$http','$route','$routeParams','$location','ngProgressFactory','localStorageService','$mdDialog','Service','salonListingSearchBoxBooleanService','$log','$q',
...
//        })
       ...
       // //console.log("these are the required lat and long---parlor detail------", localStorageService.get('requiredLat'),localStorageService.get('requiredLong'));
        ...
        ga('send', 'pageview');
        //console.log($scope.$parent.cartProduct);
                $scope.selectedDeals=$scope.$parent.cartProduct.map(function(mk){
                                                var a={services:angular.copy(mk.deals.selectors[0].services),
                                                        dealId:mk.deals.dealId,
                                                }
                                                return a;
                })
                //console.log($scope.selectedDeals)
        ...
        $http.post("/api/newDealsDetail",{selectedDeals:$scope.selectedDeals})
             .success(function(res){
                    ...
                    $scope.salons=$scope.salons.map(function(abc,dbc){
                        abc.parlorservices=[]
                            ...
                            abc.deals.forEach(function(efg){
                                 $scope.$parent.cartProduct.forEach(function(ghi){

                                            if(ghi.deals.dealId==efg.dealId){
                                                efg.name=angular.copy(ghi.deals.name);

                                            }
                                 })
                            })

                            for(var m=0;m<$scope.dummyData.length;m++)
                            {
                                // //console.log($scope.dummyData[m])
                                if($scope.dummyData[m].parlorId==abc.parlorId){
                                    //console.log(abc)
                                    abc.distance=parseFloat($scope.dummyData[m].distance)

                                    break;
                                }
                            }

                            abc.deals.forEach(function(ghijk){
                                        abc.parlorservices.push({price:ghijk.dealPrice,menuPrice:ghijk.menuPrice,name:ghijk.name})
                            })
        ...
        $scope.parlorDetails=function(value){
          $scope.$parent.passingObject.openingTime=value.openingTime;
           $scope.$parent.passingObject.closingTime=value.closingTime;
            $scope.$parent.passingObject.determinateValue=67;
            //console.log($scope.$parent.determinateValue);
            $scope.$parent.passingObject.showIt="scheduleAppointment"
        }
...
//            $scope.state = !$scope.state;
 ...
 $scope.newdealservicelist=function(){
$http.post("/api/newDealsDetail",{selectedDeals :[{dealId :36, services : [{serviceCode : 56, brandId : "saddsa", productId : "sdadsa"}]}]}).then(function(res){
     //console.log(res);
   });
}
