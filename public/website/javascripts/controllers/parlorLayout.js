...
app.controller('parlorLayout',['$rootScope','$window','$scope', '$http','$route','$routeParams','$location','ngProgressFactory','localStorageService','$mdDialog','Service','salonListingSearchBoxBooleanService','$log','$q','$timeout','$mdpDatePicker', '$mdpTimePicker', '$compile',
  function($rootScope,$window,$scope, $http,$route, $routeParams, $location,ngProgressFactory,localStorageService,$mdDialog,Service,salonListingSearchBoxBooleanService,$log,$q,$timeout,$mdpDatePicker, $mdpTimePicker, $compile) {
...
$scope.initialScreenSize=function() {
      if($scope.screenWidth>1000){
        $scope.veiwBoxValue="0 700 520";
        $scope.svgWidth=700;
      }
      else {
        $scope.veiwBoxValue="0 700 750";
        $scope.svgWidth=700;
...
$http.get("/api/parlorLayout?parlorId="+$routeParams.parlorId).success(function(response){
            console.log(response);
      if(response.data){
                 el = document.getElementById('svg');
                 angular.element(el).append( $compile(response.data.layout)($scope) );


            // $scope.initialScreenSize()
...
$scope.changeParlor=function() {
  console.log(new Date($scope.currentDate.date).toISOString());
          $http.get("/api/parlorChairStatus?parlorId="+$routeParams.parlorId+'&datetime='+ new Date($scope.currentDate.date).toISOString()).success(function(response){
            console.log(response);
