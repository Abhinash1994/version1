                               ...
                               $scope.hideNextButton=false;
                                    ...
                                    if(flag.insertionFlag)
                                {
                                    ...
                                    gtag('event', 'page_view', {
                                        ...
                                        'ecomm_prodid':$scope.cart.cartProduct[flag.index].dealId ,
                                        'ecomm_totalvalue':$scope.cart.cartProduct[flag.index].price
                                    });
                                    fbq('track', 'AddToCart',{
                                        value:$scope.cart.cartProduct[flag.index].price,
                                        currency:'INR',
                                        content_ids:[$scope.cart.cartProduct[flag.index].services[0].serviceCode],
                                ...
                                else
                                {
                                  var temporary=$scope.manageCart.refineCart(wholeObject);
                                    ...
                                    gtag('event', 'page_view', {
                                        'send_to': 'AW-870359358',
                                        'ecomm_pagetype': 'Salon Deal',
                                        'ecomm_prodid':temporary.dealId ,
                                        'ecomm_totalvalue':temporary.price
                                    });
                                     fbq('track', 'AddToCart',{
                                        value:temporary.price,
                                        currency:'INR',
                                        content_ids:[temporary.services[0].serviceCode],
                                        content_type:'Service Added To Cart'
                                    });
                                    $scope.x=2000;
                                }
...
//                ////console.log($scope.paramObj);
        ...
        $scope.additem=function(a,b,c,event){
        $scope.temporaryObject=angular.copy(b)
        $scope.temporaryObject.flag=1
           if(b.selectors[0].services.length==1 && b.selectors[0].services[0].brands.length==0)
        {
            $scope.x=2000;
                var c={services:[],type:"deals",deals:b};
                ////console.log("cart object",c);
            $scope.manageCart.insertIntoCart(c)

        }
        else
    {
            $scope.showDealDialog(event,b)
    }
    }
        ...
        .success(function(res){
            ...
            return nameParlor
                ...
                if(custom.name){
                    ...
                    $scope.salons.splice(0,0,custom.customObj)  
                }
