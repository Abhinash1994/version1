...
.controller('settlementInvoiceCtrl',['$scope','$http',
    function($scope,$http, $stateParams, $window,NgTableParams) {
        ...
        //         $scope.getLedger = function(selectedParlor) {
        //             console.log(selectedParlor)
        //        $scope.item=$scope.month[4];
        //        var month=$scope.month.indexOf($scope.item);
        //        console.log(month)
        //        var year=Number($scope.item1);
        //        console.log(year)
                     
        //     $http.get("role1/getSettlementMonthlyInvoice?parlorId="+selectedParlor+"&month="+month+"&year="+year).success(function(response, status) {
        ...
        // }
              ...
              console.log(year)
               if($scope.userType=='1'){
                url="role1/getSettlementMonthlyInvoice?parlorId="+$scope.selectedParlor+"&month="+month+ "&year="+year;
                console.log(url)
                } else{
...
//
//        
//            var printData = '<div class="row"><table class="table table-bordered"><tr><td rowspan="4"style="width:35%;text-align: left"><div style="width: 120px;float:left"><img src="http://res.cloudinary.com/dyqcevdpm/image/upload/v1526376237/beu-logo_qfscy6.png" alt="logo" style="width: 100px"></div> <div style="width:65%;float:left"><b>Gingerpan Swapcart Private Limited</b> <br>Plote No.62 Okhla Industrial Area Phase-3 Delhi<br>GSTIN/UIN:<br>State Name:<br>CIN:<br>E-mail:finance@beusalons.com</div></td><td colspan="4">Invoice No</td><td colspan="4">Date:</td></tr><tr><td colspan="4">Delevery Note</td><td colspan="4">Suppliers</td></tr><tr><td colspan="4">Delevery Note</td><td colspan="4">Suppliers</td> </tr><tr> <td colspan="4">Delevery Note</td> <td colspan="4">Suppliers</td></tr><tr><td rowspan="4" style="text-align: left"> Buyer<br>{{settlementData.parlorName}}'<br>'+settlementData.parlorAddress+'<br>'GSTIN/UIN:{{settlementData.salonGST}}<br>State Name:{{settlementData.stateName}}<br></td><td colspan="4">test</td><td colspan="4">red</td></tr><tr><td colspan="4">Delevery Note</td><td colspan="4">Suppliers</td></tr><tr><td colspan="4">Delevery Note</td><td colspan="4">Suppliers</td></tr><tr> <td colspan="4">Delevery Note</td><td colspan="4">Suppliers</td></tr><tr><td>Particulars</td><td>HSN/SAC</td><td>Quantity</td><td>Rate</td><td>Per</td><td colspan="4">Amount</td></tr><tr><td rowspan="4">Mangement Fees<br>Reimb of online payment fees<br>Reimb of online payment fees<br>CGST<br>SGST<br></td><td></td><td></td><td></td><td></td><td colspan="4"></td></tr><tr><td></td> <td></td><td></td><td></td><td colspan="4"></td></tr><tr><td></td><td></td><td></td><td></td><td colspan="4"></td></tr><tr><td></td><td></td><td></td><td></td><td colspan="4"></td></tr><tr><td>Total</td><td></td><td></td><td></td><td></td><td colspan="4">200000</td></tr><tr><td colspan="9">Amount Chargeable</td></tr><tr><td colspan="2">HSN/SAC</td><td colspan="2">Taxable<br>Value </td><td colspan="2">Central Tax</td><td colspan="2">State Tax</td><td>Total</td></tr><tr><td  colspan="2">534000</td><td  colspan="2">534000</td><td>Rate</td><td>Amount</td><td>Rate</td><td>Amount</td><td>7000</td></tr><tr><td  colspan="2">534000</td><td  colspan="2">534000</td>  <td>9</td><td>7000</td><td>9</td><td>7000</td><td>7000</td></tr><tr><td colspan="9">Tax amount in word</td></tr><tr><td colspan="5" style="text-align: left">Company Pan:<br>Declaration<br>We declare that this invoice shows the article price of the goods described and that all particular are true and correct</td><td colspan="4" style="text-align: left"> Tax amount in word</td></tr></table></div>';
//            var w = window.open();
