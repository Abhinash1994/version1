angular.module('sbAdminApp', ['isteven-multi-select'])
    .controller('repeatWiseSupport', function ($scope, $http) {
       // console.log("this is calling")

/*
     console.log("/role1/relevanceReportRepeatWise");
     $http.get("/role1/relevanceReportRepeatWise").success(function(response, status){
          console.log(response)
            $scope.repeatwise = response;
        });

*/
       // console.log("this is saolons repeat wise");
        $scope.popup1 = {
            opened: false
        };
       ...
       // console.log($scope.showMonths);
        ...
        $scope.applyFilter = function () {
           ...
           // console.log("final obj", reqObj)
            $http.post('/role1/relevanceReportRepeatWise',reqObj).success(function(response, status) {
                $scope.data = response;
               // console.log(response);
