...
angular.module('sbAdminApp', ['ui.bootstrap', 'ngAnimate', 'isteven-multi-select', 'ngSanitize'])
    .controller('newAppointmentsCtrl', function($scope, $compile, $http, $timeout, $window, $log, $rootScope, deleteAppointmentIdService) {
    ...
    $scope.showDetails=function(m){
        ...
        $scope.subscriptionRange =[]
        $scope.editCRM=editCRM;
        $scope.isSubscriptionAppointment=false;
        $scope.salonName=parlorName;
        $scope.editPriceAtBooking=editPriceAtBooking;
        $scope.membershipEmployees=[];
        ...
        $scope.activeEmployees=employees.filter(function(item) {
            return item.active;
          });
        $scope.allDPSIds = [];
        $scope.selectedUibTab = 'services';
        $scope.scheckoutPaymentOption = [];
        $scope.checkoutPaymentOptions = [{ "value": 1, "name": "Cash", "isSelected": false }, { "value": 2, "name": "Card", "isSelected": false }, { "value": 8, "name": "MX Debit Card", "isSelected": false }, { "value": 9, "name": "MX Credit Card", "isSelected": false }, { "value": 11, "name": "Affliates", "isSelected": false }, { "value": 12, "name": "Loyalty Points", "isSelected": false }, { "value": 4, "name": "Advance Online", "isSelected": false }];
        $scope.paymentOptions = [{ "value": 1, "name": "Cash", "isSelected": false }, { "value": 2, "name": "Card", "isSelected": false }, { "value": 8, "name": "MX Debit Card", "isSelected": false }, { "value": 9, "name": "MX Credit Card", "isSelected": false }, { "value": 11, "name": "Affliates", "isSelected": false }, { "value": 4, "name": "Advance Online", "isSelected": false }];
        ...
        $scope.subscriptionPaymentOptions= [{ "value": 1, "name": "Cash", "isSelected": false }, { "value": 2, "name": "Card", "isSelected": false }];
...
//    Refresh Services
        ...
        $http.get("/role3/getPriceRangeForSubscriptionPrice").success(function(response, status) {
            $scope.subscriptionRange = response.data;
            console.log("subscription data", $scope.subscriptionRange)
        });
        ...
        $http.post("/role3/presentEmployees", { 'appointmentDate': (new Date()).toJSON() }).success(function(response, status) {
        ...
        $scope.applyCoupon = function() {
            ...
            $scope.applyMenuPrice=true
            if($scope.newApp.couponCode.indexOf("SAL") > -1){
                console.log("inside personal coupon code",$scope.newApp.couponCode);
                if($scope.applyMenuPrice){
                    $scope.newApp.services.forEach(function(element) {
                        $scope.services.forEach(function(element2) {
                            if(element.serviceCode==element2.serviceCode){
                                element.serviceId=element2.serviceId
                            }
                        }, this);
                    }, this);
                }
                $http.get("/role3/getPersonalCouponDetail?couponCode="+$scope.newApp.couponCode).success(function(response, status) {
        ...
        $scope.phoneNumberChanged = function() {
            ...
            $http.post("/role3/customer", { phoneNumber: $scope.newApp.user.phoneNumber,subtotal:Math.ceil(x*1.18),noOfService:$scope.newApp.services.length }).success(function(response, status) {
                ...
                if (data) {
                    ...
                    $scope.newApp.user.subscriptionId = data.subscriptionId;
                    ...
                    $scope.newApp.user.pastAppointments = data.pastAppointments;
                    ...
                    console.log("user object",$scope.newApp.user)
                } else {
        ...
        $scope.openPDF = function(index) {
            ...
            printData += "<div style='text-align: left;line-height:-2px;'> Appointment Date: " + new Date(tobePrinted.startsAt).getDate() + "/" + new Date(tobePrinted.startsAt).getMonth() + 1 + "/" + new Date(tobePrinted.startsAt).getFullYear();
            ...
            printData += `<td>` + tobePrinted.client.name + `</td>
                     ...
                     <td>` + Math.round(tobePrinted.membersipCreditsLeft ) + `</td>`;
            ...
            if(tobePrinted.client.creditsLeft){
                message = ' Hi ' + tobePrinted.client.name + ', Thank you for availing services with Be U - ' + parlorName + ' . Your Membership Credits left are '+ tobePrinted.client.creditsLeft+' . Glad to serve you';
            }
            ...
            var twitterURL = 'https://wa.me/91'+number+'?text='+encodeURI(message);
        ...
        $scope.productNameChanged = function() {
            $scope.currProduct.name=$scope.currProduct.name.substring(0,$scope.currProduct.name.indexOf("@"))
            $scope.products.forEach(function(product) {
        ...
        $scope.publish = function(check) {
            ...
            $scope.newApp.isSubscriptionAppointment=$scope.isSubscriptionAppointment;
            ...
            $scope.newApp.services.forEach(function(ser){
                ...
                if(ser.newAdditions)ser.newAdditions=ser.newAdditions/(1 + $scope.parlorTax/100);
            })
            console.log("appointment object",$scope.subscriptionPaymentMethod,$scope.subSoldEmployees);
            if($scope.isSubscriptionAppointment){
                $http.post("/role3/subscriptionFromSalon",{phoneNumber:$scope.newApp.user.phoneNumber, amount:$scope.subscriptionPrice, firstName:$scope.newApp.user.name, emailId:$scope.newApp.user.emailId, gender:$scope.newApp.user.gender,paymentMethod:$scope.subscriptionPaymentMethod,employees:$scope.subSoldEmployees}).success(function(response, status){
                    if(response.success)    
                    {   
        ...
        function calculateTotal() {
            ...
            $scope.newApp.services.forEach(function(s) {
                ...
                if(s.newAdditions)sub=sub+s.newAdditions;
                stax += t;
                ssubtotal += sub;
            ...
            console.log("total",$scope.total,$scope.subtotal,ssubtotal,$scope.newApp.services);
            for(i=0;i<$scope.subscriptionRange.length;i++){
                console.log("total comparison",$scope.total,$scope.subscriptionRange[i],i)
                if($scope.total>$scope.subscriptionRange[i].subtotal){
                    console.log("total comparison",$scope.total,$scope.subscriptionRange[i],i)
                    $scope.subscriptionPrice=$scope.subscriptionRange[i].price;
                    if(i){
                        $scope.sdForSubscription=$scope.subscriptionRange[i-1].subtotal-$scope.total;
                        $scope.subscriptionNextSlab=$scope.subscriptionRange[i-1].price
                    }else{
                        $scope.sdForSubscription=0;
                        $scope.subscriptionNextSlab=0
                    }
                    break
                }
            }
        ...
        $scope.caClick = function(index) {
            ...
            console.log($scope.appointments1[index]);
            console.log($scope.appointments[index]);
            $scope.appointments1[index].services.forEach(function(s) {
                if (s.type != 'combo' && s.type != 'newCombo') {
                    serviceEmployeesEdit(s);
                } else {
                    console.log('newcombo kuch na ho paya');
                    /*if(s.employees.length==1){
                        comboNewComboEmployeesEdit(s);
                    }*/

                    s.services = angular.copy(s.comboServices);
                    comboNewComboEmployeesEdit(s);
                }
            ...
            console.log($scope.appointments1[index]);
            /* for(var i=0;i<$scope.appointments1[index].services;i++){
             for(var j=0;j<$scope.appointments1[index].services[i].employees.length;j++){
             if($scope.$scope.appointments1[index].services[i].employee1[0].employeeId==$scope.$scope.appointments1[index].services[i].employees[j].employeeId){
             $scope.appointments1[index].services[i].employees[j].isSelected=true;
             }
             }
             }*/
            $scope.newApp = angular.copy($scope.appointments1[index]);
            $scope.newApp.user = $scope.appointments[index].client;
            $scope.newApp.user.userId = $scope.appointments[index].client.id;
            $scope.newApp.appointmentTime = $scope.appointments[index].startsAt;
            $scope.newApp.couponCode=$scope.appointments[index].couponCode;
            $scope.newApp.aptDate = new Date($scope.appointments[index].startsAt);
            $scope.newApp.aptTime = new Date($scope.appointments[index].startsAt);
            $scope.newApp.products = $scope.appointments[index].products;
            $scope.paymentMethod = $scope.appointments[index].paymentMethod;
            for (var i = 0; i < $scope.newApp.services.length; i++) {
                $scope.newApp.services[i].eCheck = true;
            }
            if($scope.appointments[index].couponCode){
                $scope.newApp.couponCode=$scope.appointments[index].couponCode;
                $scope.newApp.couponCode01=$scope.appointments[index].couponCode;
                $scope.applyCoupon();
            }
            console.log($scope.newApp);
        };
        ...
        $scope.applyMembershipCredits = function(value) {
            ...
            console.log("appointment object for api",$scope.newApp)
            if (value) {
        ...
        $scope.isteveClick = function(index) {
            console.log(index);
            console.log($scope.newApp.services[index])
            if ($scope.newApp.services[index].employee1.length > 1) {
                $scope.revenueDistribution(index);
                $('#revenueDistribution').modal('show');
                $scope.newApp.services[index].eCheck = false;

            } else {
                $scope.newApp.services[index].eCheck = true;
                $scope.newApp.services[index].employee1[0].dist = 100;
                if ($scope.newApp.services[index].type == 'combo' || $scope.newApp.services[index].type == 'newCombo') {
                    $scope.newApp.services[index].services.find(function(serviceElement) {
                        serviceElement.employees = angular.copy($scope.newApp.services[index].employee1);
                    })
                }
        ...
        $scope.printMembership = function(pm, mem, userId) {
            ...
            var totalEmpDist=0;
            $scope.membershipEmployees.forEach(function(element) {
                totalEmpDist+=element.dist
            }, this);
            if (total == Math.ceil(mem.price * 1.18) && totalEmpDist==100) {
                ...
                $scope.data.employees=$scope.membershipEmployees
    ...
    <tr><td> (+) GST: </td><td>Rs.` + $scope.mem.price * .18 + `</td></tr>
    <tr><td> Payable Amount: </td><td>Rs.` + $scope.mem.price * 1.18 + `</td></tr>
        ...
        $scope.checkApptStatus = function(app, otp) {
            if ( app.payableAmount == 0 && otp == app.otp) {
                ...
                return false
            } else if (app.appBooking==2 && (app.allPaymentMethods.length || app.paymentMethod==5 || app.paymentMethod==10)) {
                return false;
            } else {
                return true
            }
        ...
        $scope.membershipCeilFunction=function(number,membershipId){
            var result=number*((100+$scope.parlorTax)/100)
            if(membershipId == '5c14cf5ca94e5e7ebc0a33c7' || membershipId == '5c14d1fba94e5e7ebc0ad358'|| membershipId == '5c209894a94e5e7ebc00933a'|| membershipId == '5c20add9a94e5e7ebc066d36' || membershipId == '5c46eeb3a94e5e7ebcee8a18' || membershipId == '5c46eecea94e5e7ebcee9395'){
                result=number
            }
            
            return Math.ceil(result)
        }
        $scope.employeesSelectedMembership=function(memEmployees){
            $scope.membershipEmployees=memEmployees
        }
        $scope.buySubscription=function(payment,employees){
            $scope.subscriptionPaymentMethod=payment
            $scope.subSoldEmployees=employees
            console.log("buy subsc function",payment,employees,$scope.newApp.user);
            $scope.isSubscriptionAppointment=true;
            alert("Subscription Added to user succesfully. Please collect Rs. " + $scope.subscriptionPrice);
            $('#subscriptionModal').modal('hide');

        }
        $scope.buySubscriptionModal=function(){
